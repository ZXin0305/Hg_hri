...
class ImageProcessor
{
  public:
    ...
    {
      string color_topic;   //
      this->_camera_type = nspace;  //保存相机的类型
      if(nspace == "/realSense"){
        color_topic = "/camera/color/image_raw";
      }
      else if(nspace == "/kinect2_1"){
        color_topic = ns + "/hd/image_color";
      }
      else{
        color_topic = "/kinectSDK/color";
      }
...
void ImageProcessor::loadCalibrationFiles(string& calib_path, cv::Mat& cameraMatrix, cv::Mat& distortion, double scale)
{
  ...
  if(fs.open(calib_path, cv::FileStorage::READ))
  {
    ...
    fs["camera_matrix"] >> cameraMatrix_origin;
  ...
  {
    cout << "No calibration file: calib_color.yaml, using default calibration setting" << endl;
...
void ImageProcessor::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    try
    {
      ...
      // if(this->_camera_type == "/realSense"){
...
void ImageProcessor::sendCameraTF(vector<Vec3d>& marker_trans, vector<Vec3d>& marker_rot, vector<int>& ids)
{
    ...
    if (this->_camera_type == "/realSense")
      sprintf(camera_frame_name, "realsense_%d", camera_index);
    else
...
// 基座标相对于机器人的标定，然后其他关节的就都可以通过tf读出来
...
int main(int argc, char** argv)
{
  ...
  // calib_path_1 = "/home/zx/robot_ws/src/camera_wrapper/calibration_data/kinect_2.yml";  //测试用12.28
  string calib_path_3 = "/home/zx/robot_ws/src/camera_wrapper/calibration_data";
  ImageProcessor img_processor_1(1, "/kinectSDK/color", calib_path_1);   // /kinect2_1
  // ImageProcessor img_processor_2(2, "/realSense", calib_path_2);
  ...
  if(!isCalibrationMode){
    string pose_solution_path = "/home/zx/robot_ws/src/camera_wrapper/calibration_data/solution_20180814";
